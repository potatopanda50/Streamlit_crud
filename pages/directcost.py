...
def save(dataframe):
    dataframe.to_csv('./data/direct_cost.csv', index=False)
    # st.session_state["cashsales"] = [loader('./pages/cashsales/cash_sales.csv')]

st.session_state["direct_cost_toggle"] = st.toggle('New Records')
st.caption('Always Click Save to Retain Changes')

df = loader('./data/direct_cost.csv')

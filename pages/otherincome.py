...
def save(dataframe):
    dataframe.to_csv('./data/other_income.csv', index=False)
    # st.session_state["cashsales"] = [loader('./pages/cashsales/cash_sales.csv')]

st.session_state["other_income_toggle"] = st.toggle('New Records')
st.caption('Always Click Save to Retain Changes')

df = loader('./data/other_income.csv')

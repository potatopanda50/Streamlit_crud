...
def save(dataframe):
    dataframe.to_csv('./data/accounts_payable.csv', index=False)
    # st.session_state["cashsales"] = [loader('./pages/cashsales/cash_sales.csv')]

st.session_state["accounts_payable_toggle"] = st.toggle('New Records')
st.caption('Always Click Save to Retain Changes')

df = loader('./data/accounts_payable.csv')
...
if st.session_state["accounts_payable_toggle"] == True:
    edited_df = st.data_editor(df,column_config=config,num_rows="dynamic",width=900,hide_index=True)


if st.session_state["accounts_payable_toggle"] == False:

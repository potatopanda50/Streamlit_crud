...
def save(dataframe):
    dataframe.to_csv('./data/collections.csv', index=False)

st.session_state["collections_toggle"] = st.toggle('New Records')
st.caption('Always Click Save to Retain Changes')

df = loader('./data/collections.csv')
